				...
				if ($array !== '') {
					...
					for (var i = 0; i < $array.length; i++){
						if ($array[i]['path'] !== '' && $array[i]['path'] !== undefined && $array[i]['path'] !== null && $array[i]['class'] !== undefined && $array[i]['class'] !== '') {
							...
							$html += '<li class="autocomplete-option par" onclick=goto("'+$array[i]['roll']+'");><img class="circle responsive-img" src="'+ $array[i]['pic']+'"/><span>' + $array[i]['value']+' '+$array[i]['roll'] +'</span><div style="margin-top:2px;">Downloads: <a target="_blank" class="waves-effect waves-light btn chi" href="./test.php?type=1&roll='+$array[i]['roll']+'">Friends Wrote</a>&nbsp;&nbsp;&nbsp;<a target="_blank" class="waves-effect waves-light btn chi" href="./test.php?type=2&roll='+$array[i]['roll']+'">U Wrote</a></div></li>';
						}
					}
					$html += '</ul>';
